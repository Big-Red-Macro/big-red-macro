<template>
  <div class="p-6 md:p-8 max-w-5xl mx-auto">
    ...
    <!-- Greeting header -->
    <header class="mb-8">
      <p class="text-xs font-semibold text-red-400 uppercase tracking-widest mb-1">{{ todayFormatted }}</p>
      <h1 class="text-2xl font-bold text-white tracking-tight">{{ greeting }}, Cornell!</h1>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="col-span-1 lg:col-span-2">
        ...
        <div v-if="!store.isConnectedToCalendar" class="rounded-2xl border border-slate-700 bg-slate-800 p-8 text-center">
          <h2 class="text-lg font-semibold text-white mb-2">Connect Your Calendar</h2>
        ...
        <div v-else class="space-y-4">
          <div class="flex items-center justify-between mb-2">
            <button
              ...
              class="rounded-xl border border-slate-700 bg-slate-800 hover:bg-slate-700 px-4 py-2 text-sm font-semibold text-white transition-colors disabled:opacity-50"
            >
      ...
      <div class="col-span-1 space-y-4">
        ...
        <div class="rounded-2xl border border-slate-700 bg-slate-800 p-5">
          <div class="flex items-center justify-between">
            <p class="text-xs font-bold tracking-widest text-slate-400 uppercase">Google Calendar</p>
            <span
              v-if="store.isConnectedToCalendar"
              class="rounded-full bg-emerald-500/20 px-2 py-0.5 text-[10px] font-bold text-emerald-400 uppercase tracking-wider"
            >Connected</span>
            <span
              v-else
              class="rounded-full bg-red-500/20 px-2 py-0.5 text-[10px] font-bold text-red-400 uppercase tracking-wider"
            >Offline</span>
          </div>
          <p class="mt-3 text-xs text-slate-500">Syncs daily events to find free meal gaps.</p>
        </div>
        ...
        <div class="rounded-2xl border border-slate-700 bg-slate-800 p-5">
          <div class="flex items-center justify-between mb-4">
            ...
            <button
              ...
              class="flex items-center gap-1 rounded-lg border border-slate-700 bg-slate-700/50 hover:bg-slate-700 px-2.5 py-1 text-xs font-medium text-slate-300 transition-colors disabled:opacity-50"
            >
