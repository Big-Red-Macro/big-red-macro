<template>
  <div class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
    ...
    <div class="mb-6">
      <h2 class="text-base font-bold text-slate-900 dark:text-white tracking-tight">Daily Meal Itinerary</h2>
      <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{{ date || 'Today' }}</p>
    </div>
    ...
    <!-- Timeline -->
    <div v-else-if="meals.length > 0" class="relative pl-4 border-l border-slate-200 dark:border-slate-700 ml-4 py-2 space-y-8">
      <div v-for="(meal, index) in meals" :key="index" class="relative group">
        <div class="absolute -left-[21px] top-4 h-2.5 w-2.5 rounded-full border-2 border-white dark:border-slate-900 bg-red-500"></div>
        <div class="absolute -left-10 top-3 text-xs font-semibold text-slate-400 dark:text-slate-500 w-6 text-right">
          {{ meal.meal_time?.split(' ')[0] }}
        </div>

        <div class="ml-6 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30 hover:bg-slate-100 dark:hover:bg-slate-700/50 p-4 transition-colors">
          <h4 class="text-xs font-bold tracking-widest text-red-600 dark:text-red-400 uppercase mb-2">{{ meal.dining_hall_name }}</h4>
          <ul class="text-sm text-slate-700 dark:text-slate-200 space-y-0.5">
            <li v-for="item in meal.suggested_items" :key="item" class="flex items-start gap-1.5">
              <span class="text-slate-300 dark:text-slate-600 mt-0.5">–</span>{{ item }}
            </li>
          </ul>
          <div class="flex justify-between items-end mt-3">
            <p class="text-xs text-slate-400 dark:text-slate-500 w-3/4 pr-4">{{ meal.reasoning }}</p>
            <span class="rounded-lg bg-slate-100 dark:bg-slate-900 px-2 py-1 text-xs font-semibold text-red-600 dark:text-red-400">
    ...
    <div v-if="summary" class="mt-6 rounded-xl bg-red-500/10 border border-red-500/20 px-4 py-3 text-sm text-red-600 dark:text-red-300">
      {{ summary }}
    </div>
  </div>
</template>
