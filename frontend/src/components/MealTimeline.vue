<template>
  <div class="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl relative overflow-hidden">
    ...
    <div class="absolute -top-40 -left-40 h-80 w-80 rounded-full bg-indigo-500/10 blur-[100px] pointer-events-none"></div>
    ...
    <div class="relative pl-4 border-l border-white/10 ml-4 py-2 space-y-10">
      <div v-for="(meal, index) in meals" :key="index" class="relative group">
        <!-- Dot -->
        <div class="absolute -left-[21px] top-4 h-3 w-3 rounded-full border-2 border-slate-900 bg-indigo-400 shadow-[0_0_10px_rgba(129,140,248,0.5)] group-hover:bg-rose-400 transition-colors"></div>
        ...
        <div class="ml-8 rounded-2xl border border-white/5 bg-white/5 p-4 transition-all hover:bg-white/10 hover:-translate-y-1 hover:shadow-lg">
          <div class="flex flex-col gap-2">
            <h4 class="text-xs font-bold tracking-widest text-indigo-400 uppercase">{{ meal.dining_hall_name }}</h4>
            ...
            <div class="flex justify-between items-end mt-2">
              ...
              <div class="rounded-lg bg-slate-900/50 px-2 py-1 text-xs font-semibold text-rose-300">
    ...
    <div v-if="summary" class="mt-8 rounded-xl bg-indigo-500/10 p-4 border border-indigo-500/20 text-sm text-indigo-200">
      {{ summary }}
    </div>
  </div>
</template>
